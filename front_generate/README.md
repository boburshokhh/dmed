...
2. (Опционально) Создайте файл `.env.local` в корне проекта для локальной разработки:
```env
NEXT_PUBLIC_API_URL=http://localhost:5000
```

**Примечание:** По умолчанию используется продакшн URL `https://dmed.gubkin.uz`. Для локальной разработки создайте `.env.local` с локальным URL.
